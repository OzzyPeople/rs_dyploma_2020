...
class MainRecommender:
    ...
    def __init__(self, data, weighting=True):
        ...
        # embeddings для дополнительных признаков модели
    ...
    def _get_recommendations(self, user, model, N=5):
        """Рекомендации через стардартные библиотеки implicit - ok"""
        ...
        res = [self.id_to_itemid[rec[0]] for rec in model.recommend(userid=self.userid_to_id[user],
                                                                    ...
                                                                    recalculate_user=True)] #если надо пересчитывать матрицу заново по данным пользователя
        res = [x for x in res if x != 999999]
    ...
    def get_own_recommendations(self, user, N=5):
        """Рекомендуем товары среди тех, которые юзер уже купил - ok"""
        self._update_dict(user_id=user)
    ...
    @staticmethod
    def fit(user_item_matrix, factors=30, regularization=0.001, iterations=15, num_threads=4):
        """Обучает ALS"""
